...
export function activate(context: vscode.ExtensionContext): void {
	...
	const disposable: vscode.Disposable = vscode.workspace.onDidOpenTextDocument(
		(document: vscode.TextDocument) => {
			if (document.languageId === "xml") {
				const openCDATAPrompt: Thenable<"Yes" | "No" | undefined> =
				...
				openCDATAPrompt.then((choice: "Yes" | "No" | undefined) => {
					if (choice === "Yes") {
						...
						if (settings.cdataPosition.toLowerCase() === "left") {
							setTimeout(() => {
								xmlUtilities.moveXmlFileToLastGroup(
									cdataContent.length,
									settings.openDelay,
								);
							}, settings.openDelay);
	...
	vscode.workspace.onDidChangeTextDocument(async (event: vscode.TextDocumentChangeEvent) => {
		...
		syncTimeout = setTimeout(async () => {
			if (xmlFileUri && xmlFile) {
				...
				const match = path
					.basename(event.document.fileName)
					.match(/cdata_file_(\d+)\.[^.]+$/);
